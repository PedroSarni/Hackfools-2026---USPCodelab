...
import { MATERIAL_PROOFS } from '../../shared/material-checks';
import '../styles/academy.css';

type Page = 'Hoje' | 'Missões' | 'Progresso' | 'Aula';

const MISSION_FILES: Record<string, string> = {
  'jupiter-calculo': 'calculo.html',
  'jupiter-ponteiros': 'ponteiros.html',
  'jupiter-memoria': 'memoria.html',
  'jupiter-sprint': 'sprint.html',
};
...
function Icon({ name }: { name: 'bolt' | 'coin' | 'book' | 'check' | 'lock' | 'play' }): React.JSX.Element {
...
export function App(): React.JSX.Element {
  ...
  const [material, setMaterial] = useState<Mission | null>(null);
  const [proofChecks, setProofChecks] = useState<string[]>([]);
  const [proofFile, setProofFile] = useState<File | null>(null);
  const [proofText, setProofText] = useState('');
  const [pilesReady, setPilesReady] = useState(false);
  const [videoMuted, setVideoMuted] = useState(true);
  const [videoPlaying, setVideoPlaying] = useState(true);
  const studyVideoRef = useRef<HTMLVideoElement>(null);
  const studyFrameRef = useRef<HTMLIFrameElement>(null);
  ...
  useEffect(() => {
    const receiveStudyProgress = (event: MessageEvent): void => {
      if (event.source === studyFrameRef.current?.contentWindow && event.data?.type === 'study:ready-to-complete') setPilesReady(true);
    };
    window.addEventListener('message', receiveStudyProgress);
    return () => window.removeEventListener('message', receiveStudyProgress);
  }, []);
  ...
  async function startMission(mission: Mission): Promise<void> {
    ...
    try {
      ...
      await window.baiStudyMain.materialOpened(mission.id);
      await window.baiStudyMain.openCamera();
      if (mission.id === 'jupiter-pilhas') { setPilesReady(false); setPage('Aula'); }
      else { setProofChecks([]); setProofFile(null); setProofText(''); setError(''); setMaterial(mission); }
    } catch (caught) { setError(String(caught)); }
  }
  ...
  const missionCard = (mission: Mission, featured = false): React.JSX.Element => {
    ...
    return <article className={`forced-mission ${mission.completedAt ? 'is-done' : ''} ${featured ? 'is-featured' : ''}`} key={mission.id}>
      <button className="mission-check" disabled={busy || Boolean(mission.completedAt)} onClick={() => isJupiterManaged(mission.id) ? void startMission(mission) : void dispatch({ type: 'mission.complete', id: mission.id }, `Obrigação eliminada. +${MISSION_REWARD} Study Coins.`)} aria-label={`Concluir ${mission.title}`}>
  ...
  if (material) {
    const file = MISSION_FILES[material.id];
    const proof = MATERIAL_PROOFS[material.id];
    const checksReady = proof?.items.every(item => proofChecks.includes(item)) ?? false;
    const proofPayload = proof?.method === 'file' && proofFile
      ? { method: 'file' as const, checked: proofChecks, fileName: proofFile.name, fileSize: proofFile.size }
      : proof?.method === 'text'
        ? { method: 'text' as const, checked: proofChecks, text: proofText }
        : proof?.method === 'link'
          ? { method: 'link' as const, checked: proofChecks, url: proofText }
          : { method: 'checklist' as const, checked: proofChecks };
    const proofReady = checksReady && (proof?.method === 'file' ? Boolean(proofFile) : proof?.method === 'text' ? proofText.trim().length >= 10 : proof?.method === 'link' ? /^https?:\/\//i.test(proofText.trim()) : true);
    return <main className="mission-material">
      <header><button onClick={() => setMaterial(null)}>← Voltar às atividades</button><b>{material.title}</b>
        {file && <a href={`./activities/${file}`} download>Baixar atividade</a>}
      </header>
      {file ? <iframe src={`./activities/${file}`} title={material.title} /> : <p>{material.material || 'Use seu material para concluir esta atividade.'}</p>}
      <form className="material-verification" onSubmit={event => { event.preventDefault(); if (!proofReady) return; void dispatch({ type: 'mission.complete', id: material.id, proof: proofPayload }, `Atividade comprovada! +${MISSION_REWARD} Study Coins.`); }}>
        <h2>Comprove a entrega</h2>
        {state.missions.find(m => m.id === material.id)?.completedAt ? <p role="status">Atividade concluída e moedas creditadas ✓</p> : <>
          <p>{proof?.description}</p>
          {(proof?.items ?? []).map(item => <label className="proof-check" key={item}><input type="checkbox" checked={proofChecks.includes(item)} onChange={event => setProofChecks(current => event.target.checked ? [...current, item] : current.filter(value => value !== item))}/>{item}</label>)}
          {proof?.method === 'file' && <label className="proof-file">{proof.fieldLabel}<input type="file" required accept={proof.accept} onChange={event => setProofFile(event.target.files?.[0] ?? null)}/><small>O arquivo fica no seu computador; apenas nome, tamanho e formato são conferidos.</small></label>}
          {(proof?.method === 'text' || proof?.method === 'link') && <label className="proof-text">{proof.fieldLabel}<input type={proof.method === 'link' ? 'url' : 'text'} required minLength={proof.method === 'text' ? 10 : undefined} maxLength={proof.method === 'link' ? 500 : 300} placeholder={proof.placeholder} value={proofText} onChange={event => setProofText(event.target.value)}/></label>}
          {error && <p role="alert">{error}</p>}<button disabled={busy || !proofReady}>{busy ? 'Verificando…' : 'Comprovar e concluir'}</button>
        </>}
      </form>
    </main>;
  }
  ...
  if (page === 'Aula') {
    return <main className="study-mode">
      <header><button onClick={() => setPage('Hoje')}>← Sair da aula</button><div><b>SCC0502 · PILHAS</b><span>Reels acadêmico obrigatório · Freddy e câmera ativos</span></div>
        {state.missions.find(mission => mission.id === 'jupiter-pilhas')?.completedAt
          ? <span className="study-complete">CONCLUÍDA ✓</span>
          : <button className="study-complete" disabled={!pilesReady || busy} onClick={() => void dispatch({ type: 'mission.complete', id: 'jupiter-pilhas', proof: { slidesCompleted: true } }, `Aula concluída! +${MISSION_REWARD} Study Coins.`)}>{pilesReady ? 'Marcar aula como concluída' : 'Chegue ao último slide'}</button>}
      </header>
      <div className="study-canvas">
        <iframe ref={studyFrameRef} src="./study/index.html" title="Aula de Pilhas em formato de reels com cassino de minutos" allow="autoplay" />
  ...
  return <div className={`bs theme-${state.equipped || 'default'}`}>
    <header className="topbar"><div className="topbar-inner">
      ...
      <div className="top-stats"><span><Icon name="coin" /> {balance}</span><button className="bonus-coins" title="Bônus disponível apenas na versão de desenvolvedor" disabled={busy} onClick={() => void dispatch({ type: 'wallet.bonus' }, '+100 Study Coins da versão de desenvolvedor liberadas!')}>+100 · VERSÃO DEV</button><span className="streak">🔥 {streak} dias</span><button className="camera-on" onClick={() => void window.baiStudyMain?.openCamera()}><i /> câmera ↗</button></div>
    </div></header>
    ...
    {page === 'Hoje' && <main className="content">
      ...
      <section className="mission-stack"><div className="section-title"><div><p className="eyebrow">PRA HOJE, SEM NEGOCIAÇÃO</p><h2>O algoritmo mandou.</h2></div><span>{missions.length - completed} pendentes</span></div>{missions.slice(0, 3).map((mission, index) => missionCard(mission, index === 0))}<button className="see-all" onClick={() => setPage('Missões')}>Ver grade completa →</button></section>
    </main>}

    {page === 'Missões' && <main className="content">
      <section className="page-heading"><div><p className="eyebrow">IMPORTADO AUTOMATICAMENTE</p><h1>Sua grade não aceita<br/>“depois eu vejo”.</h1></div><span className="jupiter-badge">JÚPITERWEB<br/><b>SINCRONIZADO ✓</b></span></section>
      ...
      <section className="all-missions"><div className="section-title"><div><h2>Missões obrigatórias</h2></div><span>{completed}/{missions.length} eliminadas</span></div>{missions.map((mission) => missionCard(mission))}</section>
    ...
    {page === 'Progresso' && <main className="content">
      <section className="page-heading"><div><p className="eyebrow">DADOS QUE FRED USA CONTRA VOCÊ</p><h1>Seu desempenho acadêmico.</h1></div></section>
      <section className="progress-showcase"><div className="mega-ring" style={{ '--progress': `${percent * 3.6}deg` } as CSSProperties}><div><strong>{percent}%</strong><span>SEMESTRE DOMADO</span></div></div><div className="progress-copy"><span>🔥 SEQUÊNCIA ATUAL</span><strong>{streak} dias</strong><p>Recorde pessoal: {streak + 2} dias.</p><div className="badges"><b>🦴 Fiscalizado</b><b>📚 Semestre 2</b><b>👁 Câmera online</b></div></div><img src="./fred/04_realistic_talking.gif" alt="Freddy comemorando o progresso"/></section>
